...
const IN_FILE = path.resolve(__dirname, "../../data/nvidia_offer_post_urls.txt");
const OUT_DIR = path.resolve(__dirname, "../../data/posts_offers");

// Import the extraction logic from the greedy script
import { extractPostData } from "./extract_post_details_greedy.mjs";
...
async function startBatchExtraction() {
    if (!fs.existsSync(IN_FILE)) {
        ...
        console.error("Please run collect_nvidia_urls.mjs first to collect URLs.");
        process.exit(1);
    }

    const urls = fs.readFileSync(IN_FILE, "utf-8").split("\n").filter(u => u.trim());
    console.log(`Starting batch extraction for NVIDIA company posts from: ${IN_FILE}`);
